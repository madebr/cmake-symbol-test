...
function(check_symbols_in_parallel PARALLEL_SUCCESS)
    ...
    try_compile(try_compile_result "${CMAKE_CURRENT_BINARY_DIR}/tt" SOURCES "${src_path}" OUTPUT_VARIABLE try_compile_output NO_CACHE CMAKE_FLAGS "-DCOMPILE_DEFINITIONS=${check_c_flags}" LINK_LIBRARIES "${link_libs}")
    ...
    set(sym_prefix "")
    if(".*_${non_existing_sym}.*" MATCHES re_missing_symbol_error)
        set(sym_prefix "_")
    endif()
    string(FIND "${re_missing_symbol_error}" "${sym_prefix}${non_existing_sym}" pos_non_existing_sym)
    string(FIND "${re_missing_symbol_error}" ":" pos_colon)
    while(pos_colon GREATER_EQUAL 0 AND pos_colon LESS pos_non_existing_sym)
        math(EXPR pos_after_colon "${pos_colon}+1")
        string(SUBSTRING "${re_missing_symbol_error}" ${pos_after_colon} -1 re_missing_symbol_error)
        string(FIND "${re_missing_symbol_error}" ":" pos_colon)
        string(FIND "${re_missing_symbol_error}" "${sym_prefix}${non_existing_sym}" pos_non_existing_sym)
    endwhile()
