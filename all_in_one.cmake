...
function(compare_check_symbol_checks)
    ...
    foreach(symbol IN LISTS ARG_SYMBOLS)
        ...
        if(NOT ref STREQUAL got_cxx)
            list(APPEND issues_cxx ${varname})
        endif()
        if(NOT ref STREQUAL got_linker)
            list(APPEND issues_linker ${varname})
        endif()

        message(STATUS "ref=${ref} got_cxx=${got} got_linker=${got_linker}          ${varname}")
    endforeach()
    if(issues_cxx)
        message(WARNING "These symbol checks don't match using CXX: ${issues_cxx}")
    else()
        message(STATUS "All CXX symbol checks match!")
    endif()
    if(issues_linker)
